...
from core.utils.utils import set_seed, load_jsonl
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The pal path for Auto", 
    )
    parser.add_argument(
        "--nl_path",
        type=str,
        default=None,
        help="The nl path for Auto",
    )
...
def auto(args):
    ...
    output_file = os.path.join(args.output_dir, f"{args.model}/{args.dataset}/{args.dataset_split}/{args.prompt_type}_{sd_lego_par}_{args.sampled_num}_{args.seed}_{args.temperature}_{datetime.now().strftime('%m-%d-%H-%M')}.jsonl")
    if not os.path.exists(os.path.dirname(output_file)):
        os.makedirs(os.path.dirname(output_file))
    output_writer = open(output_file, 'w', encoding='utf-8')

    # Checkpoint file to store the current index
    checkpoint_file = os.path.join(args.output_dir, f"{args.model}/{args.dataset}/{args.dataset_split}/{args.prompt_type}_{sd_lego_par}_{args.sampled_num}_{args.seed}_{args.temperature}_checkpoint.json")
    ...
    with tqdm(total=len(pl_dataset), initial=start_index) as pbar:
        for i in range(start_index, len(pl_dataset), args.batch_size):
            ...
            for j, response in enumerate(responses):
                choice = extract_choice_turbo(response.response)
                if choice == '(A)':
                    pred = nl_batch[j]['pred']
                elif choice == '(B)':
                    pred = pl_batch[j]['pred']
                else:
                    pred = None
                ...
                output = {
                    ...
                    "auto_solution": response.response,
                    "pred": pred,
